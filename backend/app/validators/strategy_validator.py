...
class StrategicValidator:
    ...
    def validate(self, brief: Brief) -> List[ValidationIssue]:
        issues: List[ValidationIssue] = []
        issues.extend(self._check_copy_differentiation(brief))
        issues.extend(self._check_budget_mix(brief))
        issues.extend(self._check_search_vs_pmax_differentiation(brief))
        return issues

    # ── Copy differentiation ───────────────────────────────────────────────────

    def _check_copy_differentiation(self, brief: Brief) -> List[ValidationIssue]:
        """
        ...
        for lang in brief.languages:
            ...
            if overlap:
                ...
                if pct >= 0.25:
                    ...
                    issues.append(ValidationIssue(
                        code="STRAT_BRAND_ACQ_COPY_OVERLAP",
                        message=(
            ...
            if has_ret and has_acq:
                ...
                if overlap_ret:
                    ...
                    if pct_ret >= 0.25:
                        issues.append(ValidationIssue(
                            code="STRAT_RET_ACQ_COPY_OVERLAP",
                            message=(
    ...
    def _check_budget_mix(self, brief: Brief) -> List[ValidationIssue]:
        """
        ...
        if brand_budget and acq_budget:
            if brand_budget.total > acq_budget.total:
                ...
                issues.append(ValidationIssue(
                    code="STRAT_BRAND_EXCEEDS_ACQ_BUDGET",
                    message=(
        ...
        if dg:
            ...
            if dg_pct < 5:
                issues.append(ValidationIssue(
                    code="STRAT_DEMAND_GEN_BUDGET_TOO_LOW",
                    message=(
    ...
    def _check_search_vs_pmax_differentiation(self, brief: Brief) -> List[ValidationIssue]:
        """
        ...
        if pmax_active and brand_active:
            ...
            if no_brand_terms:
                issues.append(ValidationIssue(
                    code="STRAT_PMAX_BRAND_CANNIBALIZATION_RISK",
                    message=(
